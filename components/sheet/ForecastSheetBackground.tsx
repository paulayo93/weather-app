...
import { Blur, Canvas, LinearGradient, Path, RoundedRect, vec } from '@shopify/react-native-skia';
...
const ForecastSheetBackground = ({width, height, cornerRadius}: ForcastSheetBackgroundProps) => { 
  ...
  return (
    <AnimatedBlurView intensity={50} tint='dark' style={[{...StyleSheet.absoluteFillObject,overflow: 'hidden'},blurViewStyles ]}>
     <Canvas style={{flex: 1}}>
        <RoundedRect x={0} y={0} width={width} height={height} r={cornerRadius}>
            <LinearGradient
            start={vec(0,0)}
            end={vec(width,height)}
            colors={['rgba(46,51,90,0.25)', 'rgba(28,57,51,0.25)']}
            positions={[-0.04, 0.95]}
